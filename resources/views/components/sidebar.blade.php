<aside id="sidebar"
    ...
    <div class="flex-1 overflow-y-auto px-3 py-4 space-y-6 sidebar-scroll">
        ...
        <nav class="space-y-1">
            ...
            <a href="{{ route('items') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium {{ $isItems ? 'text-white bg-gradient-to-r from-indigo-600/40 to-purple-600/30 border border-indigo-500/30' : 'text-slate-300 hover:text-white hover:bg-slate-800/70 transition-colors group' }}">
                <svg class="w-5 h-5 {{ $isItems ? 'text-indigo-400' : 'text-slate-400 group-hover:text-indigo-400 transition-colors' }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <nav class="space-y-1">
            ...
            tabindex="-1" 
            class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-500 opacity-50 cursor-not-allowed pointer-events-none group">
    ...
    <div id="profileMenuContainer" class="dropdown shrink-0 p-4 border-t border-slate-800/70 relative">
        ...
        <div id="profileMenu"
            ...
            @if ($isAdmin)
            <a href="{{ route('items.pending') }}" role="menuitem" tabindex="-1"
                class="dropdown-item flex items-center gap-2.5 px-2.5 py-2 rounded-lg text-xs font-medium text-amber-300 hover:text-white hover:bg-slate-800 transition-colors">
